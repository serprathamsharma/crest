...
- Kaan Can's #1 Kaggle Kernel ("Feature Selection & Data Visualization"):
  ...
  2. Joint Plots & High-Correlation Heatmap
...
- Leakage-free Stratified 5-Fold Cross-Validation using Scikit-Learn Pipelines
- Clinical Probability Threshold Optimization
- Comprehensive Diagnostic Metrics (including Matthews Correlation Coefficient)
"""
...
def load_and_prepare_data(csv_filename="data.csv"):
    ...
    print(
        f"Distribution: Benign (0): {benign_count} ({benign_count/len(y)*100:.1f}%), "
        f"Malignant (1): {malignant_count} ({malignant_count/len(y)*100:.1f}%)\n"
    )
...
def plot_correlation_heatmap(X_df):
    """
    Correlation heatmap of all 30 cell nucleus features illustrating severe multicollinearity.
    """
...
def plot_pca_2d_projection(X, y):
    """
    Dimensionality Reduction: 2D PCA projection of the 30 continuous measurements
    capturing ~63% of dataset variance and illustrating class separation.
    """
    scaler = StandardScaler()
    ...
    total_var = exp_var.sum() * 100
    plt.title(
        f"2D PCA Projection of Wisconsin Diagnostic Dataset\n"
        f"PC1 ({exp_var[0]*100:.1f}%) + PC2 ({exp_var[1]*100:.1f}%) = {total_var:.1f}% Total Variance Captured",
        fontsize=13,
        weight="bold",
        pad=12,
    )
    plt.xlabel(f"Principal Component 1 ({exp_var[0]*100:.1f}% Explained Variance)", fontsize=11)
    plt.ylabel(f"Principal Component 2 ({exp_var[1]*100:.1f}% Explained Variance)", fontsize=11)
...
def run_feature_selection_experiments(X_train, X_test, y_train, y_test, feature_names):
    ...
    rf_baseline = RandomForestClassifier(n_estimators=100, random_state=RANDOM_SEED)
    ...
    rf_kbest = RandomForestClassifier(n_estimators=100, random_state=RANDOM_SEED)
...
def build_model_zoo():
    """
    Construct the benchmark suite of diagnostic classifiers.
    """
    return {
        ...
        "Support Vector Machine (RBF)": CalibratedClassifierCV(
            SVC(kernel="rbf", C=1.0, random_state=RANDOM_SEED),
            ensemble=False,
        ),
...
def run_cross_validation_benchmark(X, y, models, cv):
    """
    Stratified 5-Fold Cross-Validation using Pipeline(StandardScaler(), model)
    to strictly prevent data leakage between training and validation folds.
    """
    scoring = ["accuracy", "recall", "precision", "f1", "roc_auc"]
    cv_records = []

    print("=" * 80)
    print("3. STRATIFIED 5-FOLD CROSS-VALIDATION BENCHMARK (Mean +/- Std, Leakage-Free Pipeline)")
    print("=" * 80)

    for name, model in models.items():
        # Pipeline encapsulates StandardScaler so each fold fits on train fold only
        pipe = Pipeline([
            ("scaler", StandardScaler()),
            ("model", model),
        ])
...
def run_holdout_evaluation(models, X_train_scaled, X_test_scaled, y_train, y_test):
    """
    Evaluate trained models on the stratified holdout test set (25%),
    computing standard clinical diagnostic metrics plus Matthews Correlation Coefficient (MCC).
    """
    all_metrics = []
    ...
    for name, model in models.items():
        ...
        # Scikit-learn unpacking: labels=[0, 1] -> TN, FP, FN, TP
    ...
    print("=" * 80)
    display_cols = ["Model", "Accuracy", "Sensitivity (Recall)", "Specificity", "Precision (PPV)", "Miss Rate (FNR)", "F1-Score", "MCC"]
    print(df_holdout[display_cols].to_string(index=False, justify="center"))
    print("=" * 80 + "\n")
    return all_metrics, df_holdout


def plot_confusion_matrices(all_metrics):
    """
    Plot and save confusion matrices comparison across all 5 models.
    """
...
def plot_roc_curves(models, X_test_scaled, y_test):
    """
    Plot and save ROC Curves for all classifiers.
    """
...
def plot_feature_importances(rf_model, feature_names):
    """
    Plot and save the top 10 feature importances from Random Forest.
    """
...
def run_clinical_threshold_tuning(rf_model, X_test_scaled, y_test):
    """
    Demonstrate clinical probability threshold tuning on Random Forest
    to reduce catastrophic False Negatives.
    """
    ...
    plt.axvline(
        ...
        linestyle=":",
        lw=2,
        label=f"Clinical Threshold ({opt_thresh:.2f} | FN={opt_fn})",
    )
...
def main():
    ...
    plot_pca_scree(X_df)
    plot_pca_2d_projection(X, y)

    # 3. Train/Test Stratified Split & Independent Preprocessing
    ...
    # 6. Stratified 5-Fold Cross-Validation (Pipeline to prevent data leakage)
    cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=RANDOM_SEED)
    run_cross_validation_benchmark(X, y, models, cv)

    # 7. Holdout Test Set Evaluation & Metric Calculation
    all_metrics, df_holdout = run_holdout_evaluation(models, X_train_scaled, X_test_scaled, y_train, y_test)
    ...
    run_clinical_threshold_tuning(models["Random Forest"], X_test_scaled, y_test)
